import wandb  # Add wandb import at the start
import argparse
import json
import os
import random
...
def main():
    ...
    # Determine the phase (train, val, or test) based on args
    phase = 'train' if not args.evaluate else 'eval'  # Adjust this based on your workflow
    
    # Get current timestamp
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    
    # Construct a run name with a timestamp suffix
    run_name = f"enc_size_{args.encoding_size}_batch_{args.batch_size}_lr_{args.lr}_{phase}_{timestamp}"
    ...
    if not args.evaluate:
        ...
        while (step <= args.train_steps):
            ...
            wandb.log({"train_loss": loss_value, "step": step})
    ...
    if args.evaluate:
        ...
        # print and log average loss values
        val_loss = np.mean(val_dict['loss_values'])
        test_loss = np.mean(test_dict['loss_values'])
        ...
        for m in ["image", "text"]:
            ...
            for ix, factor_name in factors_m.items():
                ...
                # log results for W&B
                wandb.log({
                    f"r2_linreg_{m}_{factor_name}": r2_linreg,
                    f"r2_krreg_{m}_{factor_name}": r2_krreg,
                    f"acc_logreg_{m}_{factor_name}": acc_logreg,
                    f"acc_mlp_{m}_{factor_name}": acc_mlp
                })
